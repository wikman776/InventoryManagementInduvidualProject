...
internal class ProductsManager
{
    ...
    internal static InputResult WriteTheIdOfTheProduct(List<Product> productsList, Product productToEdit)
    {
        while (true)
        {
            Console.Write("Write a 4 digit product ID: ");
            ...
            bool onlyNumbers = idInput.All(char.IsDigit);
            if (!onlyNumbers) { Helpers.ThrowErrorMessage("ID must contain numbers only, try again"); continue; }

            if (idInput.Length != 4) { Helpers.ThrowErrorMessage("ID must be exactly 4 digits, try again"); continue; }

            if (productsList.Any(product => product.ID == idInput && product != productToEdit))
            { Helpers.ThrowErrorMessage("A product with that ID already exists, try again"); continue; }
    ...
    internal static void SuccessfullyEditedProducted(Product product)
    {
        ...
        Console.WriteLine();
        ProductsManager.DisplaySingleProduct(product);
    }
    #endregion
}
